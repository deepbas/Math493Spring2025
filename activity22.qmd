...
report(ma_fit %>% select(MA3)) # Show θ coefficients
```
...
$\hat{Y}_t = \epsilon_t + 0.82\epsilon_{t-1}$ (MA(1) example) means last quarter's shock impacts current consumption by 82% of its magnitude.
