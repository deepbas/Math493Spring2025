...
1. **ARIMA Model Selection Strategy**:  
...
grid_beer <- expand_grid(
  ...
  seasonal     = list(c(0,0,0), c(1,1,0,4))
)
```
...
grid_beer <- expand_grid(
  non_seasonal = list(c(1,1,1), c(0,1,1), c(2,1,2)),
  seasonal = list(
...
results_beer %>% 
  select(ns_str, s_str, AICc) %>% 
  arrange(AICc)

# no seasonal model
no_seasonal_model <- results_beer %>% arrange(AICc) %>% slice(9)
no_seasonal_model %>% 
  pull(model) %>% 
  .[[1]] %>% 
  #residuals() %>% 
